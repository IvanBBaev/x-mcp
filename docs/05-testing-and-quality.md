...
| **Property-based (fast-check)** | encoding/ordering/algebra invariants: policy resolution (deny-wins, idempotent), `max_results` clamping across arbitrary integers *(oauth1 signature base string dropped — decision 0001)* | none / `MockAgent` | `test/prop-*.test.ts` |
