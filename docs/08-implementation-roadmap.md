...
| WP-3.3 | L | Media: chunked upload + `x_media_status`, full path-security battery, cancellation + **internal** upload progress (MEDIA-1…7). **Wording corrected 2026-08-07, then closed 2026-09-07** — this row once said "progress notifications" when only the internal seam existed. Both halves ship now: cancellation is tested end-to-end, and the per-segment seam inside `src/tools/media.ts` reaches the protocol through `src/mcp/progress.ts` (MCP-9). The bridge correlates a seam event to the `tools/call` that caused it by per-call `AbortSignal` identity — the same object the adapter already hands the registry — so overlapping uploads cannot cross-talk. A client that passes a `progressToken` gets one `notifications/progress` per accepted APPEND, counted in bytes; a client that passes none sees exactly the old behaviour, and a transport that refuses the frame never fails the upload. | Phase 2 |
