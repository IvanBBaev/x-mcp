...
> (pay-per-use reachable per the T-010 fact-check, docs/01 §3.2), so they are **not**
> availability-gated — they register by default and are guarded by the credit budget,
> not hidden. docs/02 §5.1, docs/08 (WP-0.3), and docs/09 (T-012 row) carry the same
> reconciled numbers.

<!-- GENERATED:TOOLS:BEGIN -->

<!-- Generated by scripts/docs-gen.mjs from the live registry — do not edit by hand. -->
...
- **Cost** column — indicative pay-per-use bucket; the **authoritative price table
  ...
  - `r:post` ~$0.005/post · `r:user` ~$0.010/user · `r:follows` ~$0.010 ·
    `r:list` ~$0.005 (list metadata/timelines) · `r:engage` ~$0.001 (like/mute/block
...
- **Phase** column — the roadmap phase a tool ships in (docs/08). `P1` read-only
  ...
  availability independently. Retagged 2026-07-31 (exit-gate-2 audit, T-214):
  `x_post_hide_reply` and `x_bookmarks_list` moved P2 → P3 — WP-2.3 (docs/08) never
  scoped them for Phase 2, and this column follows the roadmap. Both shipped in
  Phase 3 (2026-08-09); the four engagement read tools retagged alongside them were
  cut instead — see [Deliberate omissions](#deliberate-omissions).
...
| `x_usage_get` | read:account | app+user | owned | P3 | X (Twitter): this cycle's post-read consumption vs the monthly project cap, with optional per-day / per-app breakdown (`GET /2/usage/tweets`, `days` 1–90) + local session-spend budget state. **GO 2026-07-31 (WP-3.11):** ships unconditionally — reachable under pay-per-use with an app bearer token, no tier gate (docs/01 §3.5). Reports post-read **counts, never dollars** (X publishes no spend API), so it does not seed the session credit estimate (COST-2) |
...
| `x_bookmarks_list` | read:content | user-only | owned | P3 | X (Twitter): own bookmarks. `max_results` 1–100 |
...
| `x_dm_send` | write:dm | user-only | w:dm | P3 | X (Twitter): send a DM to a conversation or directly to a user id. Subject to the ~1,440/24h send cap (corner case DM-4). **Never enabled by any preset — including `full`; requires an explicit `X_MCP_POLICY_ALLOW="write:dm"` override** |
...
- **Nine design-catalogue rows cut on 2026-08-09**
  ([decisions/0002](decisions/0002-remaining-catalogued-tools.md)) — they were
  catalogued in the v1 design but never implemented, and shipping them would have
  spent most of the remaining `tools/list` context budget (docs/02 §5.1) on tools an
  agent can already reach another way:
  - `x_liked_posts_list`, `x_liking_users_list`, `x_reposted_by_list`,
    `x_quote_posts_list` — engagement audience reads. Every one is a
    conversation/author query `x_search_recent` already answers
    (`conversation_id:`, `quotes_of:`), at the same per-post price.
  - `x_blocks_list`, `x_mutes_list` — own moderation lists. The writes
    (`x_block_set`, `x_mute_set`) are absolute, not toggles, so an agent never needs
    to read the list to act correctly.
  - `x_space_get`, `x_spaces_search` — Spaces. Audio is outside what a text-oriented
    server can usefully render, and the endpoints return metadata only.
  - `x_trends_by_location` — trends. Needs a WOEID table this server would have to
    carry and keep current; personalized trends are `premium-user`, a class no
    shipped tool uses.

  Cutting a row is reversible: the names stay reserved, and a future minor can add
  any of them back. What is **not** reversible is shipping one and then removing it,
  which is why they are cut rather than left standing as unimplemented rows.
