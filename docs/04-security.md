...
|---|---|---|---|
| T1 | Token theft from disk | Token file readable by other users/processes | Token file created `0600`; startup warns (and `auth_status` reports) if perms are wider; the token dir must not be group/other-writable (§4.2); Keychain backend via `X_MCP_TOKEN_KEYCHAIN` (Phase 3) |
| T2 | Token leak via logs/output | Tokens echoed into MCP results or stderr | Tokens never enter tool results; the log layer never constructs lines from secrets and redacts token forms defense-in-depth (§6); `auth_status` shows scopes and user, never token material |
...
| T6 | Wrong-account writes | Multiple profiles, agent assumes wrong identity | One profile per process; `auth_status` leads with `@handle`; write results echo the acting account |
| T7 | Budget/financial exhaustion | Agent burns paid reads/writes or triggers overage | Session credit budget + typed `budget` error (operator-set, model-immutable); no auto-pagination; preemptive rate-limit refusal; platform "out of credits" mapped to `billing` |
...
- **Operation**: `read` → `write` → `destructive`. `destructive` = delete / **block** —
  ...
  is `destructive:*`.
- **Domain**: `content` (posts, lists, media) · `engagement` (likes, reposts,
  bookmarks) · `social-graph` (follow/block/mute) · `dm` · `moderation` · `account`.
...
- **No unlock hint for sensitive cells.** For `dm`, `destructive:*`, and `social-graph`
  cells the error names the cell **without** the env var/value that would unlock it — it
  must never hand the model an escalation recipe it can relay to the operator (see
  Scenario C, finding F10). Low-sensitivity cells (e.g. `write:engagement`) *may* include
  the unlock hint.
...
> Scenario C. The rule above supersedes it: sensitive-cell errors name the cell only.
...
- The same `0600`-and-warn discipline extends to the **profiles file** and any OAuth1 /
...
- The `Authorization` header (or OAuth1 signature) is attached **only** when the resolved
  request host is on a **hardcoded allowlist**: `api.x.com`, `upload.x.com`, plus the
  explicit dev-override host from CFG-7. For any other host the header is **not** attached.
...
- Rotate (revoke + re-authorize) tokens after any suspected leak; `auth_status` shows the
  token file location and permissions, and — under OAuth1 — that scopes are not enforced.
...
**Verdict.** **Broken at the machine boundary and, for sensitive cells, closed at the human
boundary.** The design no longer hands the attacker the escalation script.
